...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const filtered = ALL_RESTAURANTS.filter(
      (res) =>
      ...
      {isOpen && (
        <div className="border-t border-white/10 bg-[#090b12]/98 md:hidden">
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            <Link
              href="/"
              className="block text-white hover:bg-gray-900 hover:text-orange-400 px-3 py-2 rounded-md font-medium"
            >
              Inicio
            </Link>
            <Link
          ...
          <div className="px-5 py-4 border-t border-gray-800 flex flex-col space-y-3">
            {isLogged ? (
              <button
                onClick={() => logoutUser()}
                className="text-gray-400 hover:text-white transition w-full text-left px-3 py-2"
              >
                Cerrar Sesión
              </button>
            ) : (
              <>
                <Link href="/login" className="w-full">
                  <button className="bg-transparent text-white px-6 py-2.5 rounded-full font-medium border border-gray-600 w-full">
                    Iniciar Sesión
                  </button>
                </Link>
                <Link href="/register" className="w-full">
                  <button className="bg-linear-to-r from-orange-500 to-pink-500 text-white px-6 py-2.5 rounded-full font-medium w-full">
                    Registrarse
                  </button>
                </Link>
              </>
